	...
	static void Stopped(int TrainNo, String station, double timestamp,
		...
		for (int i = 0; i < Passenger.TotalNumOfPassenger_Central; i++) {
			...
			if (Passenger.ListOfPassenger_Central.get(i).getSrc()
					...
					&& (k2 > k1) && Direction == "up"&& timestamp>Passenger.ListOfPassenger_Central.get(i).nextJourneyTime) {
				...
				for(int b=0;b<Passenger.faultyPassenger.length;b++){
					if(Passenger.faultyPassenger[b]==Passenger.ListOfPassenger_Central.get(i).id){
						faulty=true;
						break;
					}
			...
			//	val = (int) inp.nextDouble();
				...
				countWalkIn++;
					...
					.equals(station)
					&& Passenger.ListOfPassenger_Central.get(i).TrainNo == -1
					&& (k2 < k1) && Direction == "down"&& timestamp>Passenger.ListOfPassenger_Central.get(i).nextJourneyTime) {
				// add that passenger to train list
				Passenger.ListOfPassenger_Central.get(i).setTrainNo(TrainNo);
				Passenger.ListOfPassenger_Central.get(i).setStatus("InTrain");
				Trains.Trainlist_Central.get(TrainNo - 1).Train
...
//setting next journey time
	...
	public static void getSpottingsNow(double nowtime) {
		 ...
		 */
		ArrayList<Train_Spotting> Train_Spotting_List_Central_t = new ArrayList<Train_Spotting>(
				10000);
		
		
		for(Train_Spotting p : Train_Spotting.Train_Spotting_List_Central) {
			Train_Spotting_List_Central_t.add(p.clone(p.Id,p.Timestamp,p.DistFromOriginMeter,p.Confidence,p.DistNow,p.Direction,p.Station));
		}
		...
		//	 System.out.println("m="+m);
			...
			/*
			for(int kk=0;kk<Station.StationList_Central.size();kk++){
				System.out.print(Station.StationList_Central.get(kk).getNextStationDistance());
			}
			System.out.println();
			*/
		...
		PosnConf.PosnConfidnce_List_Central_Up.clear();
		PosnConf.PosnConfidnce_List_Central_Down.clear();
		...
		Train_Spotting.Train_Spotting_List_Central.clear();
		for(Train_Spotting p : Train_Spotting_List_Central_t) {
			Train_Spotting.Train_Spotting_List_Central.add(p.clone(p.Id,p.Timestamp,p.DistFromOriginMeter,p.Confidence,p.DistNow,p.Direction,p.Station));
		}
	}
